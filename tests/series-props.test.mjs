...
test("a row becomes the template's props", () => {
  ...
  assert.deepEqual(p.endcard, { clip: "assets/3d/endcard-join-010010-100bpm-7b.mp4", mode: "join" });
});

test("every short's end card fuses the yin lines in turn, whatever the track", () => {
  for (const t of ["kun", "gen", "qian", "li", "zhen", "kan", "xun", "dui"]) assert.equal(seriesProps({ ...row, upper: t }).endcard.mode, "join");
});
...
test("an override that changes the tempo changes the clip too", () => {
  ...
  assert.equal(p.endcard.clip, "assets/3d/endcard-join-010010-82.5bpm-7b.mp4");
});
