...
const renderOne = async (n) => {
  ...
  const files = Object.fromEntries(
    [props.music, `local/music/certificates/${row.music.certificate.file}`, props.hexagramClip, props.endcard.clip, ...props.screens.map((s) => s.src), ...props.plates].map((f) => [f, sha256(f)]),
  );
